...
1Panel 本地应用：基于 [项目主页](<!-- TODO: 由协调者填入 -->) 的 Infinity 推理服务，兼容 OpenAI Embeddings API，可在 1Panel 内一键部署，自动批处理与动态批大小，支持 bge / e5 / qwen 等主流嵌入模型。
...
2. 找到 **Infinity** → 点击 **安装**
3. 填写安装表单：
   - **Docker 镜像**：保持默认 `infinity-pascal:cu124`（本地构建镜像，详见 [docs/infinity/BUILD.md](../infinity/.docs/BUILD.md)）
   ...
   - **宿主机 API 端口**：默认 `7997`
4. 点击 **部署**，等待容器启动
5. 验证服务：浏览器访问 `http://<host>:7997/health`，或运行 [scripts/infinity/verify.sh](../infinity/.scripts/verify.sh)
...
> 说明：Pascal（sm_61）是默认构建的最低目标，`int8` 是其兼容性最佳选择；Ampere / Hopper 可用更高精度获取更好效果。不同模型的显存占用参考 [docs/infinity/SUPPORTED_MODELS.md](../infinity/.docs/SUPPORTED_MODELS.md)。
...
> 也支持任意自定义 HuggingFace repo id（填入 `EMBEDDING_MODEL` 即可）。更详细的维度 / dtype / batch / 显存建议见 [docs/infinity/SUPPORTED_MODELS.md](../infinity/.docs/SUPPORTED_MODELS.md)。
...
> 注意：AnythingLLM 的 Embedding 维度需与所选模型维度一致（见 [docs/infinity/SUPPORTED_MODELS.md](../infinity/.docs/SUPPORTED_MODELS.md)）。
...
详见 [docs/infinity/KNOWN_ISSUES.md](../infinity/.docs/KNOWN_ISSUES.md)。
...
构建策略（wheels 来源、Git LFS / Release 附件 / 外部下载源）详见 [docs/infinity/BUILD.md](../infinity/.docs/BUILD.md)。

## 相关链接 / Related Links

- 项目主页：[<!-- TODO: 由协调者填入 -->](<!-- TODO: 由协调者填入 -->)
- Docker Hub：[`michaelf34/infinity`](https://hub.docker.com/r/michaelf34/infinity)
- GitHub：[<!-- TODO: 由协调者填入 -->](<!-- TODO: 由协调者填入 -->)
...
A 1Panel local app for the [project](<!-- TODO: 由协调者填入 -->) Infinity inference server. OpenAI-compatible Embeddings API, one-click deploy inside 1Panel, with automatic and dynamic batching. Supports bge / e5 / qwen and other mainstream embedding models.
...
3. Fill in the install form:
   - **Docker Image**: keep `infinity-pascal:cu124` (locally built image, see [docs/infinity/BUILD.md](../infinity/.docs/BUILD.md))
...
5. Verify: visit `http://<host>:7997/health`, or run [scripts/infinity/verify.sh](../infinity/.scripts/verify.sh)
...
> Pascal (sm_61) is the baseline build target; `int8` is the best compatibility choice there. Ampere / Hopper can use higher precision. See [docs/infinity/SUPPORTED_MODELS.md](../infinity/.docs/SUPPORTED_MODELS.md) for per-model VRAM guidance.
...
Any custom HuggingFace repo id is also supported via `EMBEDDING_MODEL`. See [docs/infinity/SUPPORTED_MODELS.md](../infinity/.docs/SUPPORTED_MODELS.md) for dtype / batch / VRAM details.
...
> The embedding dimension must match the model (see [docs/infinity/SUPPORTED_MODELS.md](../infinity/.docs/SUPPORTED_MODELS.md)).
...
See [docs/infinity/KNOWN_ISSUES.md](../infinity/.docs/KNOWN_ISSUES.md).
...
Build strategies (wheels source, Git LFS / Release asset / external download) are in [docs/infinity/BUILD.md](../infinity/.docs/BUILD.md).

## Related Links

- Project home: [<!-- TODO: 由协调者填入 -->](<!-- TODO: 由协调者填入 -->)
- Docker Hub: [`michaelf34/infinity`](https://hub.docker.com/r/michaelf34/infinity)
- GitHub: [<!-- TODO: 由协调者填入 -->](<!-- TODO: 由协调者填入 -->)
